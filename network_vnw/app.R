...
server <- function(input, output) {
	...
	output$network = renderVisNetwork({
	  ...
	  g = graph_from_data_frame(d = edges, directed=TRUE, vertices = nodes)
		...
		subTitle = "Ties: teachers and students"
		
		labsize <- rescale(degree(g_), min(degree(g_)), max(degree(g_)), 0.2, 2.0)
		...
		visNetwork(nodes = data$nodes, edges = data$edges, main = "Ties: teachers")%>%
	...
	observe({
  	...
  	# id of vertex required to export properly to pajek
  	V(g)$id = V(g)$name
