...
```{r}
h4("Upload data set")
fileInput("NodesInput", 
          "Choose CSV Nodes File", 
          accept = ".csv")

fileInput("EdgesInput", 
          "Choose CSV Edges File", 
          accept = ".csv")
```
Row
-----------------------------------------------------------------------
...
```{r} 
nodeDataSet <- eventReactive(input$NodesInput,{nodeDataSet <- read.csv(input$NodesInput$datapath)})
renderTable({
...
```{r}
EdgeDataSet <- eventReactive(input$EdgesInput,{EdgeDataSet <- read.csv(input$EdgesInput$datapath)})
renderTable({
...
output$travels_network = renderLeaflet({
  ...
  tcu_map = "https://api.mapbox.com/styles/v1/kwalkertcu/ckb83x2ur02oa1hpiq9zdt1i2/tiles/{z}/{x}/{y}?access_token=pk.eyJ1Ijoia3dhbGtlcnRjdSIsImEiOiJjaW9jenN1OGwwNGZsdjRrcWZnazh2OXVxIn0.QJrmnV9lJzdXHkH95ERdjw"
