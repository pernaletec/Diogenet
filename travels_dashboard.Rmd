...
######################################################################
#read nodes file
nodes_LOPI <- read.csv(file="new_Nodes_Life_of_Pythagoras_Iamblichus.csv", header = TRUE, sep = ",", encoding = "UTF-8", stringsAsFactors = FALSE)
# read edges file
edges_LOPI <- read.csv("new_Edges_Life_of_Pythagoras_Iamblichus.csv", header = TRUE, sep = ",", encoding = "UTF-8", stringsAsFactors = FALSE)
...
output$download_edges_graph <- renderUI( {
  if (input$dataset_selection == 'diogenes') {
    downloadButton("downBtnEdgesGraphDiogenes", "Download .csv", style = "width:100%;")
  } else if (input$dataset_selection == 'LOPI') {
    downloadButton("downBtnEdgesGraphLOPI", "Download .csv", style = "width:100%;")
  }
  
})

# Add download handling
diogenes <- read.csv("travel_edges_graph_diogenes.csv")
LOPI <- read.csv(file = "travel_edges_graph_LOPI.csv")
output$downBtnEdgesGraphDiogenes <- downloadHandler(
  filename = function() {"travel_edges_graph_diogenes.csv"},
  content = function(file) {write.csv(diogenes, file, row.names = TRUE)}
)
output$downBtnEdgesGraphLOPI <- downloadHandler(
  filename = function() {"travel_edges_graph_LOPI.csv"},
  content = function(file) {write.csv(LOPI, file, row.names = TRUE)}
)
...
graph_reactive = reactive({
    ...
    write.csv(travel_edges, "travel_edges_graph_diogenes.csv")
    write.csv(all_places, "all_places_graph_diogenes.csv")
...
graph_reactive_LOPI <- reactive({
  ...
  write.csv(travel_edges, "travel_edges_graph_LOPI.csv")
  write.csv(all_places, "all_places_graph_LOPI.csv")
