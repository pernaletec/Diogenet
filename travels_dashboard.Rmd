...
travel_edges_reactive = reactive({
  ...
  if ((input$phylosopher != "All Phylosophers") && !is.null(input$phylosopher)) {
...
observe({
  ...
  #all_places_full_data = all_places_full_data_reactive()
  ...
  leafletProxy("travels_network") %>% addCircleMarkers(all_places_full_data$lon,
...
output$table_values = DT::renderDataTable({datatable(values()[order(values(), decreasing = TRUE),], selection = 'none', caption = 'Centrality scores', 
                                   options = list(
                                                 initComplete = JS(
