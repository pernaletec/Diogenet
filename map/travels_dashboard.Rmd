...
###################### 3. Create a dataframe with the follow form:  source target name lat_source lat_targer lon_source lon_target #########
  ...
  #all_places_full_data = read.csv(file = "./../data/locations_data.csv", header = TRUE, sep = ",", dec = ".", stringsAsFactors = FALSE)
...
CentralityDataTable <- function (graph) {
  # This function take a graph object as argument and return a DataTable with centrality scores (degree, betweenness, closeness, eigenvector)
  return(
    DT::renderDataTable({datatable(graph[order(graph$degree, decreasing = TRUE),], selection = 'none', caption = 'Centrality scores', 
                                   options = list(
                                                 initComplete = JS(
...
CentralizationDataTable <- function (graph) {
  # This function take a graph object as argument and return a DataTable with centralization scores (degree, betweenness, closeness, eigenvector)
  return(
   DT::renderDataTable({datatable(graph, selection = 'none', caption = 'Centralization scores',
...
black_list = read.csv(file="./../data/travels_blacklist.csv", header = FALSE, sep = ",", encoding = "UTF-8", stringsAsFactors = FALSE)
...
nodes_diogenes = read.csv(file="./../data/new_Nodes.csv", header = TRUE, sep = ",", encoding = "UTF-8", stringsAsFactors = FALSE)
# Reads the edges file
edges_diogenes = read.csv(file="./../data/new_Edges.csv", header = TRUE, sep = ",", encoding = "UTF-8", stringsAsFactors = FALSE)
######################################################################
###############  Life of Pythagoras Iamblichus Dataset ###############
######################################################################
#read nodes file
nodes_LOPI <- read.csv(file="./../data/new_Nodes_Life_of_Pythagoras_Iamblichus.csv", header = TRUE, sep = ",", encoding = "UTF-8", stringsAsFactors = FALSE)
# read edges file
edges_LOPI <- read.csv("./../data/new_Edges_Life_of_Pythagoras_Iamblichus.csv", header = TRUE, sep = ",", encoding = "UTF-8", stringsAsFactors = FALSE)
...
# Travel edges for Life Of Pythagoras Iamblichus (LOPI) dataset
travel_edges_LOPI <- travelsData(nodes_LOPI, black_list, all_places_full_data, edges_LOPI)
...
selectInput(inputId= "dataset_selection", 
            ...
            choices = c("Diogenes Laertius" = "diogenes", "Iamblichus" = "LOPI"),
            selected = "diogenes")

# Reactive menu "Select a Traveler" is shown based on the selected dataset
...
# h5("Custom Travel Edges Dataset")
# 
# # file Input button for custom dataset
...
br()
h5("Download travel edges graph data")
uiOutput("download_edges_graph")
...
travel_edges_custom <- reactive({
    ...
    if (is.null(inFile))
      return(NULL)
    # fileInput parameters validation
    file_contents <- read.csv(inFile$datapath, header = TRUE, sep = ",", encoding = "UTF-8", stringsAsFactors = FALSE)
    required_columns <- c("source","target","name","lat_source","lon_source","lat_target","lon_target")
    column_names <- colnames(file_contents)
    max_columns <- 8
    # fileInput data content validation 
    if (all(required_columns %in% column_names)){
      df <- file_contents
      return(df)
    } else if (ncol(file_contents) <= max_columns){
      return(NULL)
    } else {
      return(NULL)
    }
...
# the "custom" option is shown in the selectInput "select a dataset" if a .csv file is uploaded
 observe({
    x <- travel_edges_custom()

    # Can use character(0) to remove all 
    if (!is.null(x)){
      updateSelectInput(session, "dataset_selection",
      label = "select a dataset",
      choices = c("Diogenes Laertius" = "diogenes", "Iamblichus" = "LOPI", "Custom" = "custom"),
      selected = "diogenes"
      )
    }
...
output$travelerInput <- renderUI({
  ...
  if (input$dataset_selection == 'diogenes') {
    selectInput(inputId = "philosopher", 
                ...
                selected = "All")
  } else if (input$dataset_selection == 'LOPI') {
    selectInput(inputId = "philosopher_LOPI", 
                label = "Select a Traveler", 
                choices = c("All", sort(travel_edges_LOPI$name, decreasing = FALSE)), 
                selected = "All")
...
# Create the actual downloadButton for travel edges graph data
output$download_edges_graph <- renderUI( {
  downloadButton("downBtnEdgesGraph", "Download .csv", style = "width:100%;")
})

# Add download handling
datadownlad_reactive <- reactiveFileReader(intervalMillis = 1000, session = NULL, filePath = 
"./../data/travel_edges_graph.csv", readFunc = read.csv)
observe({
  datadownlad <- datadownlad_reactive()
  output$downBtnEdgesGraph <- downloadHandler(
    filename = function() {"Travel Edges Graph.csv"},
    content = function(file) {write.csv(datadownlad, file, row.names = TRUE)}
  )
})
...
# =======
# Reactive function that filter edges depending on the Phylosopher selected
# If 'All' is selected all edges are considered for the graph what means all 
# travelers 
...
# Reactive travel edges expression for  Life of pythagroas Iamblichus dataset
travel_edges_LOPI_reactive <- reactive({
    # Filter by Phylosopher selected...if any
    if ((input$philosopher_LOPI != "All") && !is.null(input$philosopher_LOPI)) {
      travel_edges_selected <- travel_edges_LOPI$name %in% input$philosopher_LOPI
      return(travel_edges_LOPI[travel_edges_selected,])
    } else return(travel_edges_LOPI)
})
...
# =======
# Depending on the travelers selected places are ordered stored in a vector
...
# Reactive places expression for Life of pythagroas Iamblichus dataset
all_places_LOPI_reactive <- reactive({
  travel_edges_LOPI <- travel_edges_LOPI_reactive()
  all_places_LOPI <- sort(unique(c(travel_edges_LOPI$source,
                                   travel_edges_LOPI$target)),
                          decreasing = FALSE)
  return(data.frame(places_LOPI = all_places_LOPI))
})
...
# =======
# A comparisson is made between all the places for which a location is available
# with the places selected to be included in the graph
...
# Reactive all places full data expression for Life of Pythagoras Iamblichus dataset
all_places_full_data__LOPI_reactive <- reactive({
  all_places_LOPI <- all_places_LOPI_reactive()
  drawn_places_LOPI <- all_places_full_data$name %in% all_places_LOPI
  return(all_places_full_data[drawn_places_LOPI,])
})
...
# Reactive graph expression for Life of Pythagoras Iamblichus dataset
graph_reactive_LOPI <- reactive({
  travel_edges <- travel_edges_LOPI_reactive()
  all_places <- all_places_LOPI_reactive()
...
output$travels_network = renderLeaflet({
  ...
  m <- leaflet() %>% 
    addTiles(urlTemplate = tcu_map, attribution = map_attr, options = list(minZoom = 3, 
                                                                           maxZoom = 6,
...
observe({
  ...
  if (input$dataset_selection == 'LOPI') {
    travel_edges <- travel_edges_LOPI_reactive()
    all_places <- all_places_LOPI_reactive()
    graph <- graph_reactive_LOPI()
    values = values_LOPI()
    write.csv(travel_edges, "./../data/travel_edges_graph.csv")
    write.csv(all_places, "./../data/all_places_graph.csv")
...
values_LOPI <- reactive({ CentralityValues(graph_reactive_LOPI()) })
...
# Reactive centralization values expression for Diogenes dataset
centr <- reactive({ CentralizationValues(graph_reactive()) })
# Reactive centralization values expression for Life of phytagoras dataset 
centr_LOPI <- reactive({ CentralizationValues(graph_reactive_LOPI()) })
# Reactive centralization values expression for Custom dataset 
centr_custom <- reactive({ CentralizationValues(graph_reactive_custom()) })

############### centrality Data Table ########################
# Diogenes dataset
output$table_values <- CentralityDataTable(values())
# Life of phytagoras Iamblichus Dataset
output$table_values_LOPI <- CentralityDataTable(values_LOPI())
# Custom Dataset
output$table_values_custom <- DT::renderDataTable({datatable(values_custom()[order(values_custom()$degree, decreasing = TRUE),], 
                                   selection = 'none', caption = 'Centrality scores', 
                                   options = list(
                                                 initComplete = JS(
                                                  "function(settings, json){",
                                                  "$(this.api().table().body()).css({'font-size': '14px'});",
...
############### centralization Data Table ########################
# Diogenes dataset
output$table_centr <- CentralizationDataTable(centr())
# Life of phytagoras dataset
output$table_centr_LOPI <- CentralizationDataTable(centr_LOPI())
# Custom dataset
...
output$dynamic_table <- renderUI({ 
  if (input$dataset_selection == 'LOPI') {
      fillCol(DT::DTOutput(outputId = 'table_centr_LOPI'),
        br(),
        DT::DTOutput(outputId = 'table_values_LOPI'), flex = c(NA,NA,NA))
  }  else if (input$dataset_selection == 'diogenes') {
...
# Life of phytagoras Iamblichus Dataset
output$graph_map_LOPI <- visNetwork::renderVisNetwork({
  
  d = graph_reactive_LOPI()
...
output$dynamic_graph <- renderUI({ 
  if (input$dataset_selection == 'LOPI') {
   visNetworkOutput(outputId="graph_map_LOPI")
  }  else if (input$dataset_selection == 'diogenes') {
