...
output$comm_network = renderVisNetwork({
  ...
  # If there is only one edge selected and this edge is the number 1, use directed edges
  if(length(input$edges_select_comm) == 1 & input$edges_select_comm[1] == 1) directed = TRUE else directed = FALSE
  ...
  validate(
    need(try(!is.null(cluster)), 
         paste("The algorithm ",
               input$cmnt_dtc_alg,
               " could not identify communities given the current Phylosophers and Relations")
         )
  )
