...
output$local_network_degree = renderVisNetwork({
  ...
  if (!is.null(input$node_sel_m) && !is.null(input$order_m)) {
    ...
    output$table_centr = renderDT(return(centr), options = list(autoWidth = TRUE))
    
    output$table_centr_ = renderTable({return(centr)}, align = 'c', width = NULL)
...
```{r}
  fillCol(h5("Graph metrics"),
          tableOutput(outputId = 'table_centr_'),
