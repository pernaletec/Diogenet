...
output$comm_network = renderVisNetwork({
  ...
  # if (input$cmnt_dtc_alg == "Cluster Edge Betweenness") try(cluster <- cluster_edge_betweenness(g_))
  # if (input$cmnt_dtc_alg == "Cluster Fast Greedy") try(cluster <- cluster_fast_greedy(g_))
  # if (input$cmnt_dtc_alg == "Cluster Infomap") try(cluster <- cluster_infomap(g_))
  # if (input$cmnt_dtc_alg == "Cluster Label Prop") try(cluster <- cluster_label_prop(g_))
  # if (input$cmnt_dtc_alg == "Cluster Leading Eigen") try(cluster <- cluster_leading_eigen(g_))
  # if (input$cmnt_dtc_alg == "Cluster Louvain") try(cluster <- cluster_louvain(g_))
  # if (input$cmnt_dtc_alg == "Cluster Optimal") try(cluster <- cluster_optimal(g_))
  # if (input$cmnt_dtc_alg == "Cluster Spinglass") try(cluster <- cluster_spinglass(g_))
  # if (input$cmnt_dtc_alg == "Cluster Walktrap") try(cluster <- cluster_walktrap(g_))
