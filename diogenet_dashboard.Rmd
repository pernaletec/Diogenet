...
View {data-navmenu="Local Network"}
===================================== 
...
output$local_network = renderVisNetwork({
  ...
  if (!is.null(input$node_sel) && !is.null(input$order)) {
    ...
    values =  data.frame(name = vertex_attr(d[[1]],"id"),
                         degree = degree(d[[1]]), 
                         betweenness = betweenness(d[[1]]),
                         closeness = closeness(d[[1]]) , 
                         eigenvector = eigen_centrality(d[[1]])$vector)
    
    centr = data.frame(name = vertex_attr(d[[1]],"id"),
                       centr_dgre = centralization.degree(d[[1]]),
                       centr_btwn = centralization.betweenness(d[[1]]), 
                       centr_clsn = centralization.closeness(d[[1]]), 
                       centr_egcn = centralization.evcent(d[[1]]))
    
      output$table_values = renderDT({
        return(values)
      }, options = list(scrollX = TRUE, scrollY = TRUE))
      
      output$table_centr = renderDT({
        return(centr)
      }, options = list(scrollX = TRUE, scrollY = TRUE))
...
<!-- DTOutput(outputId = 'table_values') -->
...
<!-- DTOutput(outputId = 'table_centr') -->

<!-- ``` -->

Info {data-navmenu="Local Network" data-orientation="columns"}
===================================== 
...
output$local_network_m = renderVisNetwork({
  ...
  if (!is.null(input$node_sel_m) && !is.null(input$order_m)) {
    ...
    # Use function is_directed
    directed = is_directed(input$edges_select_local_m)
    ...
    #values =  data.frame(name = vertex_attr(d[[1]],"id"),
    values =  data.frame(degree = degree(d[[1]]), 
                         betweenness = format(betweenness(d[[1]]), digits = 3, nsmall = 3),
                         closeness = format(closeness(d[[1]]), digits = 3, nsmall = 3), 
                         eigenvector = format(eigen_centrality(d[[1]])$vector, digits = 3, nsmall = 3))
    
    values = values[order(values$degree, decreasing = TRUE),]
    
    centr = data.frame(centr_dgre = centralization.degree(d[[1]])$centralization[1],
                       centr_dgre_th_max = centralization.degree(d[[1]])$theoretical_max[1],
                       centr_btwn = centralization.betweenness(d[[1]])$centralization[1], 
                       centr_btwn_th_max = centralization.betweenness(d[[1]])$theoretical_max[1], 
                       centr_clsn = centralization.closeness(d[[1]])$centralization[1], 
                       centr_clsn_th_max = centralization.closeness(d[[1]])$theoretical_max[1], 
                       centr_egcn = centralization.evcent(d[[1]])$centralization[1], 
                       centr_egcn_th_max = centralization.evcent(d[[1]])$theoretical_max[1])
    
    # Centralization
    output$cent_degree = renderValueBox({
      valueBox(value = format(centr$centr_dgre, digits = 3, nsmall = 3), caption = "Degree", icon = "fa-sitemap")  
    }) 
    
    output$cent_betwns = renderValueBox({
      valueBox(value = format(centr$centr_btwn, digits = 3, nsmall = 3), caption = "Betweenness", icon = "fa-users")  
    }) 
          
    output$cent_clsnes = renderValueBox({
      valueBox(value = format(centr$centr_clsn, digits = 3, nsmall = 3), caption = "Closeness", icon = "fa-link")  
    }) 
                
    output$cent_evcent = renderValueBox({
      valueBox(value = format(centr$centr_egcn, digits = 3, nsmall = 3), caption = "Eigenvector centralities", icon = "fa-user-circle")  
    }) 
    
    #Theoretical max
    
    # output$cent_degree_th_max = renderValueBox({
    #   valueBox(value = centr$centr_dgre_th_max,caption = "<p>Centralization degree <br> <small>Theoretical Max</small><p>",icon = "fa-cloud-download-alt", color = "info")  
    # }) 
    # 
    # output$cent_betwns_th_max = renderValueBox({
    #   valueBox(value = centr$centr_btwn_th_max,caption = "<p>Centralization betweenness <br> <small>Theoretical Max</small><p>",icon = "fa-cloud-download-alt", color = "info")  
    # }) 
    #       
    # output$cent_clsnes_th_max = renderValueBox({
    #   valueBox(value = format(centr$centr_clsn_th_max, digits = 3, nsmall = 3),caption = "<p>Centralization closeness <br> <small>Theoretical Max</small><p>",icon = "fa-cloud-download-alt", color = "info")  
    # }) 
    #             
    # output$cent_evcent_th_max = renderValueBox({
    #   valueBox(value = centr$centr_egcn_th_max,caption = "<p>Centralization eigenvector centralities <br> <small>Theoretical Max</small><p>",icon = "fa-cloud-download-alt", color = "info")  
    # }) 
  
    output$table_values = renderDT(return(values), options = list(pageLength = 5, paging = TRUE, scrollY = TRUE))
...
visNetworkOutput(outputId="local_network_m")
...
output$local_network_degree = renderVisNetwork({
  ...
  if (!is.null(input$node_sel_m) && !is.null(input$order_m)) {
    ...
    n_col =  max_deg - min_deg + 1
    node_vir_col = viridis(n = n_col, begin = 0.2, end = 0.9, direction = -1, option = "A")
    ...
    # Setting parameters straight in the data frame for visNetwork
    data$edges$color.color = rep("lightgray",length(data$edges$Relation))
    
    data$edges$color.highlight = rep("lightgray",length(data$edges$Relation))
...
visNetworkOutput(outputId="local_network_degree")
...
output$local_network_betweenness = renderVisNetwork({
  ...
  if (!is.null(input$node_sel_m) && !is.null(input$order_m)) {
    ...
    # Set node size
    if (is.null(input$node_size_local_m[1])) min_node = rescaled_value(3.0,30.0,4)
    else min_node = rescaled_value(3.0,30.0,input$node_size_local_m[1])

    if (is.null(input$node_size_local_m[2])) max_node = rescaled_value(3.0,30.0,6)
    else max_node = rescaled_value(3.0,30.0,input$node_size_local_m[2])
...
visNetworkOutput(outputId="local_network_betweenness")
...
output$local_network_closeness = renderVisNetwork({
...
visNetworkOutput(outputId="local_network_closeness")
...
output$local_network_centralities = renderVisNetwork({
...
visNetworkOutput(outputId="local_network_centralities")
...
### Centralization_degree {.value-box}

```{r}
valueBoxOutput("cent_degree")
```

### Centralization_betweenness {.value-box}

```{r}
valueBoxOutput("cent_betwns")
```

### Centralization_closeness {.value-box}

```{r}
valueBoxOutput("cent_clsnes")
```

### Centralization_evcentralities {.value-box}

```{r}
valueBoxOutput("cent_evcent")
```

<!-- Row  -->
<!-- ------------------------------------- -->

<!-- ### Centralization_degree_th_max {.value-box} -->

<!-- ```{r} -->
<!-- valueBoxOutput("cent_degree_th_max") -->
<!-- ``` -->

<!-- ### Centralization_betweenness_th_max {.value-box} -->

<!-- ```{r} -->
<!-- valueBoxOutput("cent_betwns_th_max") -->
<!-- ``` -->

<!-- ### Centralization_closeness_th_max {.value-box} -->

<!-- ```{r} -->
<!-- valueBoxOutput("cent_clsnes_th_max") -->
<!-- ``` -->

<!-- ### Centralization_evcentralities_th_max {.value-box} -->

<!-- ```{r} -->
<!-- valueBoxOutput("cent_evcent_th_max") -->
<!-- ``` -->

### Metrics by Phylosopher 
