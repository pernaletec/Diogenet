...
network_cmnt =  reactive({
  ...
  if (input$cmnt_vsl_pck == "igraph") {
      ...
      # Set node size
      if (is.null(input$nodeUI_igp[1])) min_node = 8.0
      else min_node = input$nodeUI_igp[1]
      
      if (is.null(input$nodeUI_igp[2])) max_node = 20.0
      ...
      output$cmnt_igp = renderPlot({
        withProgress(message = 'Creating graph', style = 'notification', value = 0.1, {
          ...
          mod <- round(modularity(cluster),3)
          subTitle = paste0("MODULARITY: ", mod)
          return(plot(cluster,g_,main = subTitle))
      })
    })
...
output$nodeUI_igp <- renderUI({
  if (input$cmnt_vsl_pck == "igraph") {
    if (!is.null(input$nodeUI_igp)) sliderInput(inputId = "nodeUI_igp", label = "Node Size", min = 3, max = 30, value = c(input$nodeUI_igp[1], input$nodeUI_igp[2]),ticks = FALSE, step = 1)
    else sliderInput(inputId = "nodeUI_igp", label = "Node Size", min = 3, max = 30, value = c(8, 20),ticks = FALSE)
  }
})
