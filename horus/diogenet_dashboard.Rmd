...
# sourceDataNodes <- function(input){
...
#   nodesData <- read.csv(paste0("./../data/", nodePath), 
#                   sep=",", 
#                   header = TRUE, 
...
#   edgesData <- read.csv(paste0("./../data/", edgesPath),
#                   header = TRUE, 
#                   sep=",",
...
#     } else message = ""
#   
#   #horus_data <- list("nodes" = nodesData, "edges" = edgesData)
#   return(nodesData)
# }
# 
# sourceDataEdges <- function(input){
#   dataset <- read.csv('./../data/datasetList.csv',
#                   header = TRUE,
#                   sep=",",
#                   col.names = c("name","path","type"),
...
nodes <- reactive({
  ...
  # Remove leading and trailing whitespace
  nodes$Name <- stri_trim(nodesData$Name)
  nodes$Group <- stri_trim(nodesData$Group)
  edges$Source <- stri_trim(edges$Source)
  edges$Target <- stri_trim(edges$Target)
  edges$Relation <- stri_trim(edges$Relation)
  ...
  edges_ = c(edges$Source,edges$Target)
  length(edges_)
  missing = edges_ %in% nodesData$Name
  missing_indx = which(missing == FALSE)
  edges_[missing_indx]
  table(edges_ %in% nodesData$Name)
  
  # Discard edges whith unknown nodes 
  if (length(missing_indx) >0) {
    missing_edges = nodesData[unique(c(which(nodesData$Source %in% edges_[missing_indx]), which(edges$Target %in% edges_[missing_indx]))),]
    edges = edges[-unique(c(which(edges$Source %in% edges_[missing_indx]), which(edges$Target %in% edges_[missing_indx]))),]
    message = paste("WARNING: Check data. ", length(missing_indx)," edges removed", ": ") 
    for (k in 1:length(missing_edges$Source)) message = paste(message,",", missing_edges[k,1],"-",missing_edges[k,3],"-", missing_edges[k,2])
    } else message = ""
...
# observe({
#   req(input$dataset_selection)
#   selection <- input$dataset_selection
#   data <- sourceData(selection)
#   # print(head(data$nodes))
